...
while True:
    ...
    if pick == 1:
        ...
        if data_ops.check_file_exist(data_file):
            data_ops.get_count_letter(data_file)
        else:
            print("brak danych do sprawdzenia")
        # 2. Zlicz liczbę liter w pobranym pliku
    elif pick == 3:
        if data_ops.check_file_exist(data_file):
            count_words = data_ops.get_count_words(data_file)
        ...
        if data_ops.check_file_exist(data_file):
            count_signs = data_ops.get_count_punctuation(data_file)
        ...
        if data_ops.check_file_exist(data_file):
            count_sentences = data_ops.get_count_sentences(data_file)
        ...
        if data_ops.check_file_exist(data_file):
            data_ops.get_letters_report(data_file)
